...
public class VehicleAddSummary extends BaseService {
	String date = DateFormatUtils.format(new Date(), "yyyy-MM-dd");

	@Test
	public void vehAddSummary() throws Exception {
		entryPage("统计报表", "新增统计");
	...
	private void exportData() throws Exception {
		...
		assertEquals("新增车辆", getCellValue1("sheet1", 1, 2));
		assertEquals("新增用户", getCellValue1("sheet1", 1, 5));
		assertEquals("已绑定车机数量", getCellValue1("sheet1", 1, 8));
		assertEquals("解绑车机数量", getCellValue1("sheet1", 1, 9));
		assertEquals("新增绑定设备", getCellValue1("sheet1", 1, 10));
	}
	...
	protected void compareTest(int a, int b, int c) throws InterruptedException {
		List<WebElement> elements = dr.findElements(By.xpath("//tr[1][@class='gradeA']/td"));

		int value1 = Integer.valueOf(elements.get(a).getText());
		int value2 = Integer.valueOf(elements.get(b).getText());
		int value3 = Integer.valueOf(elements.get(c).getText());
